...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="font-h1 text-h1 text-on-surface">Tambah Tugas Baru</h1>
        <p class="font-body-sm text-body-sm text-outline mt-1">Isi detail untuk tugas barumu</p>
    </div>
        ...
        <form action="{{ route('todos.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea class="w-full px-4 py-3 bg-surface-bright border border-surface-variant rounded-lg
                          ...
                          placeholder="Tambahkan deskripsi tugas...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-error">{{ $message }}</p>
                @enderror
            </div>
